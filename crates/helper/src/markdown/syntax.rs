...
//
// Adaptation delta vs upstream (crates/syntax/src/lib.rs @ 0.2.28):
// - `thiserror` dropped (zero-dep error type with manual Display/Error).
// - Grammar set reduced to the bundled subset (rust/js/jsx/ts/tsx/python/go/
//   json/jsonc/bash/toml/yaml/css/html); every other LanguageId variant
//   reports GrammarUnavailable and markdown code blocks fall back to plain
//   text. Binary size was the driver — the full set is ~25 grammars.
// - Markdown as a highlight PARENT is dropped: pulldown-cmark owns the
//   document parse here, and code fences resolve to their language directly
//   (Comet's injected_languages(Markdown) machinery is unnecessary).
// - Html keeps its js/css/json injections (embedded scripts/styles).
// - HighlightedDocument::from_absolute_spans is kept public: its tests and
//   the render integration both consume line-relative spans.

//! Syntax highlighting for markdown code blocks: tree-sitter under a
//! small, stable contract. Public ranges are byte offsets relative to one
//! UTF-8 source line.
...
impl LanguageId {
    /// Whether this build bundles a parser and compatible highlight queries.
...
impl std::fmt::Display for HighlightError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnknownLanguage => write!(f, "the source language is not registered"),
            Self::InvalidRange { start, end, len } => write!(
                f,
                "highlight range {start}..{end} is invalid for a {len}-byte source"
            ),
            Self::InvalidUtf8Boundary { start, end } => write!(
                f,
                "highlight range {start}..{end} is not on UTF-8 boundaries"
            ),
            Self::SourceTooLarge => write!(f, "source exceeds the configured highlighting limit"),
            Self::TooManySpans => write!(f, "highlight output exceeds the configured span limit"),
            Self::Parser(message) => write!(f, "parser failed: {message}"),
            Self::GrammarUnavailable(language) => {
                write!(f, "the {language:?} grammar is not bundled")
            }
        }
    }
}
...
impl HighlightedDocument {
    /// Validate, split, and normalize absolute source spans into
    /// line-relative spans.
...
/// Highlight a complete document with explicit limits and cooperative
/// cancellation.
pub fn highlight_with_limits(
    ...
    // Html is the only bundled parent with injections (script/style
    // blocks). Markdown never reaches this path here: pulldown-cmark owns
    // document parsing and code fences resolve to their language directly.
    let injected = if matches!(language, LanguageId::Html) {
...
fn injected_languages(parent: LanguageId) -> Vec<LanguageId> {
    match parent {
        LanguageId::Html => vec![LanguageId::JavaScript, LanguageId::Css, LanguageId::Json],
        _ => Vec::new(),
    }
}
...
fn rust_configuration() -> Result<HighlightConfiguration, HighlightError> {
    ...
    // `constant.builtin`; the port preserves those structural roles
    // separately (upstream Comet does the same rewrite).
...
fn configuration(language: LanguageId) -> Result<HighlightConfiguration, HighlightError> {
    ...
    match language {
        ...
        // Unbundled in this build (see the header delta): unreachable via
        // highlight_with_limits, which checks is_bundled() first.
        Markdown | C | Cpp | CSharp | Java | Kotlin | Swift | Ruby | Php | Sql | Lua
        | Dockerfile | Nix | Make => Err(HighlightError::GrammarUnavailable(language)),
    }
}
...
mod tests {
    ...
    fn spans_are_valid_sorted_non_overlapping_and_line_relative() {
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            HighlightError::InvalidUtf8Boundary { start: 8, end: 9 }
        );
    }
...
fn build(value: usize) -> Widget {
    ...
    fn limits_and_unbundled_languages_degrade_with_typed_errors() {
        ...
        // Registered but not bundled in this build: typed error, caller
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            HighlightError::GrammarUnavailable(LanguageId::C)
        );
    }
    ...
    #[test]
    fn every_bundled_grammar_loads_and_highlights_a_fixture() {
        let fixtures = [
            ...
            (LanguageId::Toml, "Cargo.toml", "name = \"solid-gpui\""),
            (LanguageId::Html, "index.html", "<main id=\"app\"></main>"),
            (LanguageId::Css, "app.css", ".app { color: red; }"),
            (LanguageId::Yaml, "app.yml", "name: solid-gpui"),
        ];
